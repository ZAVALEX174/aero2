...
function createOrUpdateAirVolumeText(line) {
  ...
  const airVolumeText = new fabric.Text(`${formatTo5(line.properties.airVolume)} м³/с`, {
    ...
    evented: false,
    originX: 'center',
    textBaseline: 'alphabetic', // Явно задаем допустимое значение
    originY: 'center',
    angle: degrees,
...
// ==================== СЕТКА ====================
// В функции drawGrid обновите расчет размеров:
...
function createIntersectionPoint(x, y, index, intersectionData, customColor = '#ff4757') {
  ...
  const text = new fabric.Text((index + 1).toString(), {
    ...
    evented: false,
    originX: 'center',
    textBaseline: 'alphabetic', // Добавляем
    originY: 'center',
...
function exportLinePropertiesToCSV() {
  ...
  function exportToPDF() {
    ...
    const scale = 2; // Увеличиваем качество в 2 раза
    ...
    try {
      ...
      if (wasGridVisible) {
        toggleGrid(); // Включаем сетку обратно
      }
  ...
  function exportToPDFAdvanced(options = {}) {
    ...
    html2canvas(canvasElement, {
      ...
      try {
        ...
        if (format === 'original') {
          // Оригинальный размер
          const imgWidthMM = (canvas.width * 25.4) / 96; // Конвертация px в mm
          ...
          pdf.addImage(canvas, 'PNG', 0, 0, imgWidthMM, imgHeightMM);
        } else {
          // Стандартные форматы
          ...
          const [pageWidth, pageHeight] = formatDimensions[format] || [210, 297];
...
window.addEventListener('error', function (e) {
  if (e.message.includes('CanvasTextBaseline') || e.message.includes('alphabetical')) {

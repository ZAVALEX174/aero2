...
console.log("07/02/2026 10-30 - Версия с неразрывными соединениями линий");
...
const APP_CONFIG = {
  ...
  MAX_IMAGE_SIZE: 40,
  NODE_THRESHOLD: 5, // Расстояние для определения узлов
  NODE_LOCK_DEFAULT: true // Узлы заблокированы по умолчанию
};
...
// ==================== НОВЫЕ ПЕРЕМЕННЫЕ ДЛЯ УЗЛОВ ====================
let connectionNodes = new Map(); // Хранит информацию о соединениях
...
/**
 * Построение графа соединений линий
 */
function buildConnectionGraph() {
  ...
  lines.forEach(line => {
    // Используем округленные координаты для создания ключей
    ...
    // Проверяем, нет ли уже этой линии в узле
  ...
  // Очищаем узлы с одной линией (не являются соединениями)
...
/**
 * Отрисовка визуальных маркеров узлов
 */
function drawConnectionNodes() {
  ...
  // Рисуем новые маркеры только для узлов с 2+ линиями
  connectionNodes.forEach((node, key) => {
    if (node.lines.length >= 2) {
      ...
      // Текст с количеством соединений
...
/**
 * Поднимает маркеры узлов на передний план
 */
...
function startNodeDrag(nodeKey, clientX, clientY) {
  ...
  // Сохраняем оригинальные позиции линий
...
function updateNodeDrag(mouseX, mouseY) {
  ...
  // Обновляем координаты ВСЕХ линий в узле
  affectedLines.forEach(lineInfo => {
    ...
    // Обновляем тексты объемов воздуха
...
function endNodeDrag() {
  ...
  // Обновляем граф соединений
  updateLineConnections();

  // Пересчитываем точки пересечения
...
/**
 * Обновление всех соединений линий
 */
...
/**
 * Переключение режима блокировки узлов
 */
...
/**
 * Проверка, находится ли точка в заблокированном узле
 */
...
/**
 * Получение информации о ближайшем узле
 */
...
function setupCanvasEvents() {
  ...
  // События мыши для узлов (через глобальные обработчики)
  setupNodeMouseEvents();
}

function setupNodeMouseEvents() {
  // Глобальные обработчики для перетаскивания узлов
...
function handleCanvasMouseDown(options) {
  ...
  // Если уже перетаскиваем узел, игнорируем остальные события
  if (isDraggingNode) {
  ...
  // Проверяем, не нажали ли мы на узел (маркер или близко к узлу)
  const target = options.target;
  if (target && target.type === 'line') {
    // Проверяем, близко ли клик к существующему узлу
    const nearestNode = getNearestNode(pointer.x, pointer.y);
    if (nearestNode && nearestNode.distance < APP_CONFIG.NODE_THRESHOLD * 2) {
      // Клик близко к узлу - начинаем перетаскивание
      options.e.preventDefault();
...
function splitAllLines() {
  ...
  intersections.forEach((inter, index) => {
    if (inter.line1 && inter.line2) {
      // Проверяем, не находятся ли точки в заблокированных узлах
    ...
    images.forEach(image => {
      ...
      if (closestPoint.param >= 0 && closestPoint.param <= 1) {
        ...
        if (distanceToCenter <= tolerance) {
          // Проверяем, не находится ли точка в заблокированном узле
...
function deleteCurrentObject() {
  ...
  // Если это линия, проверяем, не находится ли она в заблокированном узле
...
function setupKeyboardShortcuts() {
  document.addEventListener('keydown', function (event) {
    ...
    if (event.key === 'Delete') {
      const activeObject = canvas.getActiveObject();
      if (activeObject) {
        // Проверяем, не находится ли линия в заблокированном узле
...
function testNodeConnection() {
  ...
  // Создаем тестовые линии, соединенные в узле
  const line1 = new fabric.Line([100, 100, 200, 100], {
    stroke: '#4A00E0',
    strokeWidth: 5,
    properties: { name: 'Линия 1', airVolume: 10 }
  });

  const line2 = new fabric.Line([200, 100, 200, 200], {
    stroke: '#4A00E0',
    strokeWidth: 5,
    properties: { name: 'Линия 2', airVolume: 5 }
  });

  const line3 = new fabric.Line([200, 100, 300, 100], {
    stroke: '#4A00E0',
    strokeWidth: 5,
    properties: { name: 'Линия 3', airVolume: 5 }
  });
  ...
  fabric.Image.fromURL(testImage.path, function (img) {
    ...
    setTimeout(() => {
      ...
      showNotification('Тестовая сцена создана. Попробуйте переместить узел в точке (200,100)', 'success');
...
console.log('Редактор технических чертежей с неразрывными соединениями полностью загружен!');
...
console.log('- updateLineConnections() - обновление соединений линий');
console.log('- toggleNodeLock() - переключение блокировки узлов');
console.log('- testNodeConnection() - тест неразрывных соединений');
console.log("07/02/2026 10-30 - Оптимизированная версия с улучшенной производительностью");
...
function calculateAirVolumesForAllLines(isManual = false) {
  ...
  if (isCalculatingAirVolumes) {
    ...
    if (Date.now() - calcStartTime > 10000) {
  ...
  return calculateWithMainThread();
}

function calculateWithMainThread() {
  const startTime = performance.now();
  let calculationTimeout = false;

  const timeoutId = setTimeout(() => {
    calculationTimeout = true;
    console.warn('Расчет прерван по таймауту');
  }, APP_CONFIG.MAX_CALCULATION_TIME);
  ...
  try {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      nodes.get(startKey).outgoing.push({
        line: line,
        resistance: line.properties?.airResistance || 1
      });

      nodes.get(endKey).incoming.push({
        line: line,
        resistance: line.properties?.airResistance || 1
      });
    }
    ...
    const sources = [];
    for (const [key, node] of nodes.entries()) {
      if (node.object && node.object.properties?.airVolume !== undefined && node.object.properties.airVolume > 0) {
        sources.push({
          node: node,
          volume: roundTo5(node.object.properties.airVolume)
        });
      }
    ...
    function calculateFlowIterative(startNode, startVolume) {
      const stack = [{
        node: startNode,
        volume: startVolume,
        visited: new Set()
      ...
      while (stack.length > 0) {
        const {node, volume, visited} = stack.pop();

        if (visited.has(node) || calculationTimeout) {
          continue;
        }

        visited.add(node);
        const volumeAfterObject = roundTo5(volume / node.objectResistance);
        ...
        for (let i = 0; i < node.outgoing.length; i++) {
          ...
          if (edge.resistance > 0) {
            const conductivity = roundTo5(1 / edge.resistance);
            edge.conductivity = conductivity;
            conductivities.push({edge, conductivity});
            totalConductivity += conductivity;
          }
        }

        for (let i = 0; i < conductivities.length; i++) {
          const {edge, conductivity} = conductivities[i];
          if (edge.resistance > 0 && totalConductivity > 0) {
            const edgeKey = `${edge.line.id}_${edge.line.x1}_${edge.line.y1}_${edge.line.x2}_${edge.line.y2}`;
            if (processedEdges.has(edgeKey)) continue;

            const lineVolume = roundTo5(volumeAfterObject * (conductivity / totalConductivity));

            if (edge.line.properties) {
              edge.line.properties.airVolume = roundTo5((edge.line.properties.airVolume || 0) + lineVolume);
              edge.line.set('properties', edge.line.properties);
            }

            processedEdges.add(edgeKey);

            const endKey = `${roundTo5(edge.line.x2)}_${roundTo5(edge.line.y2)}`;
    ...
    for (let i = 0; i < sources.length; i++) {
      if (calculationTimeout) break;
      calculateFlowIterative(sources[i].node, sources[i].volume);
    }
    ...
    if (duration > 1000) {
      console.warn(`Расчет занял ${(duration / 1000).toFixed(2)} секунд`);
    }

    if (updatedCount > 0) {
      showNotification(`Расчет завершен! Обновлено ${updatedCount} линий за ${duration.toFixed(0)}ms`, 'success');
    } else {
...
// ==================== ИСПРАВЛЕННЫЕ ФУНКЦИИ ПЕРЕСЕЧЕНИЙ ====================
function findAllIntersections() {
  ...
  // Временный массив для быстрого доступа
  const tempLines = [...lines];

  // Поиск пересечений линий
  for (let i = 0; i < tempLines.length; i++) {
    for (let j = i + 1; j < tempLines.length; j++) {
      const intersection = lineIntersection(tempLines[i], tempLines[j]);
      if (intersection) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (let j = 0; j < images.length; j++) {
      ...
      // Проверяем пересечение линии с прямоугольником объекта
      const intersectionPoints = getLineRectIntersection(line, rect);
      ...
      // Дополнительно проверяем центр объекта
...
// Исправленная функция пересечения линий
function lineIntersection(line1, line2) {
  if (line1 === line2) return null;

  const x1 = line1.x1, y1 = line1.y1;
  const x2 = line1.x2, y2 = line1.y2; // ИСПРАВЛЕНО: было line2.y2
...
// Новая функция для пересечения линии с прямоугольником
function getLineRectIntersection(line, rect) {
  const intersections = [];

  // Стороны прямоугольника
  const sides = [
    {x1: rect.left, y1: rect.top, x2: rect.right, y2: rect.top},    // верх
    {x1: rect.right, y1: rect.top, x2: rect.right, y2: rect.bottom}, // право
    {x1: rect.right, y1: rect.bottom, x2: rect.left, y2: rect.bottom}, // низ
    {x1: rect.left, y1: rect.bottom, x2: rect.left, y2: rect.top}   // лево
  ];
    ...
    if (intersection) {
      ...
      if (length > 0) {
        const param = Math.sqrt(
          Math.pow(intersection.x - line.x1, 2) +
          Math.pow(intersection.y - line.y1, 2)
        ) / length;
...
// Функция пересечения двух отрезков
function segmentIntersection(seg1, seg2) {
  ...
  if (ua >= 0 && ua <= 1 && ub >= 0 && ub <= 1) {
    const x = x1 + ua * (x2 - x1);
    const y = y1 + ua * (y2 - y1);
    return {x: roundTo5(x), y: roundTo5(y)};
  }
    ...
    if (points.length > 0) {
      ...
      for (const point of points) {
        ...
        if (Math.abs(point.distance) > 0.1 &&
          Math.abs(point.distance - line.length) > 0.1) {
...
// ==================== ИСПРАВЛЕННЫЕ ТОЧКИ ПЕРЕСЕЧЕНИЯ ====================
...
function collectPointInfo(x, y) {
  ...
  lines.forEach(line => {
    const distanceToStart = roundTo5(Math.sqrt(
      Math.pow(x - line.x1, 2) + Math.pow(y - line.y1, 2)
    ));

    const distanceToEnd = roundTo5(Math.sqrt(
  ...
  return {
    ...
    linesEnding: linesInPoint.filter(l => !l.isStart).length
  };
}
    ...
    pointData.linesInPoint.forEach((lineInfo, index) => {
      ...
      html += `
        <li>
          ...
          <span class="detail">${lineInfo.isStart ? '✅ НАЧАЛО линии' : '⭕ КОНЕЦ линии'}</span><br>
          <span class="detail">Длина: ${formatTo5(length)} px</span><br>
          <span class="detail">Объем воздуха: ${formatTo5(lineInfo.airVolume || 0)} м³/с</span><br>
    ...
    pointData.objectsInPoint.forEach((objInfo, index) => {
      html += `
        <li>
          ...
          <span class="detail">Объем воздуха: ${formatTo5(objInfo.airVolume || 0)} м³/с</span><br>

// console.log("08/02/2026 11-45 - Исправленная версия с корректным расчетом воздушных потоков");
...
//       nodes.get(startKey).outgoing.push(edge);
//       nodes.get(endKey).incoming.push(edge);
//     });
//
//     // 3. ПРИВЯЗКА ОБЪЕКТОВ К УЗЛАМ
//     images.forEach(img => {
...
//
//       if (closestNode) {
//         closestNode.object = img;
//         closestNode.objectResistance = roundTo5(img.properties?.airResistance || 1);
//
//         // Если у объекта задан объем воздуха - это источник
//         if (img.properties?.airVolume !== undefined && img.properties.airVolume > 0) {
//           closestNode.isSource = true;
//           closestNode.sourceVolume = roundTo5(img.properties.airVolume);
//           console.log(`Найден источник в узле ${closestNode.id}: ${closestNode.sourceVolume} м³/с`);
//         }
//       }
//     });
//
...
//       if (node.isSource) {
//         sources.push(node);
//         console.log(`Источник в точке (${node.x}, ${node.y}): ${node.sourceVolume} м³/с`);
//       }
//     });
//
//     if (sources.length === 0) {
//       showNotification('Не найдено источников воздуха (объектов с объемом воздуха > 0)', 'warning');
//       isCalculatingAirVolumes = false;
//       return false;
//     }
//
//     // 5. РАСЧЕТ ПОТОКОВ ОТ ИСТОЧНИКОВ
//     const processedNodes = new Set();
//
//     function calculateFlowFromSource(sourceNode, availableVolume) {
//       if (availableVolume <= 0) return;
//
//       console.log(`Расчет от источника ${sourceNode.id} с объемом ${availableVolume}`);
//
//       // Учитываем сопротивление объекта
//       let volumeAfterObject = availableVolume;
//       if (sourceNode.objectResistance > 0) {
//         volumeAfterObject = roundTo5(availableVolume / sourceNode.objectResistance);
//         console.log(`После объекта (R=${sourceNode.objectResistance}): ${volumeAfterObject}`);
//       }
//
//       // Распределяем по исходящим линиям
//       if (sourceNode.outgoing.length === 0) return;
//
//       // Суммируем обратные сопротивления (проводимости)
//       let totalConductivity = 0;
//       sourceNode.outgoing.forEach(edge => {
//         if (edge.resistance > 0) {
//           edge.conductivity = 1 / edge.resistance;
//           totalConductivity += edge.conductivity;
//         }
//       });
//
//       console.log(`Всего исходящих линий: ${sourceNode.outgoing.length}, суммарная проводимость: ${totalConductivity}`);
//
//       if (totalConductivity <= 0) return;
//
//       // Распределяем поток пропорционально проводимости
//       sourceNode.outgoing.forEach(edge => {
//         if (edge.resistance > 0) {
//           const conductivity = 1 / edge.resistance;
//           const flow = roundTo5(volumeAfterObject * (conductivity / totalConductivity));
//
//           edge.flow = flow;
//           if (edge.line.properties) {
//             edge.line.properties.airVolume = flow;
//             edge.line.set('properties', edge.line.properties);
//           }
//
//           console.log(`Линия ${edge.id} получает: ${flow} м³/с (R=${edge.resistance}, проводимость=${conductivity})`);
//
//           // Переходим к следующему узлу
//           const nextNode = nodes.get(edge.to);
//           if (nextNode && !processedNodes.has(nextNode.id)) {
//             processedNodes.add(nextNode.id);
//             calculateFlowFromSource(nextNode, flow);
//           }
//         }
//       });
//     }
//
//     // Запускаем расчет от каждого источника
//     sources.forEach(source => {
//       if (!processedNodes.has(source.id)) {
//         processedNodes.add(source.id);
//         calculateFlowFromSource(source, source.sourceVolume);
//       }
//     });
//
//     // 6. ОБРАБАТЫВАЕМ УЗЛЫ С НЕСКОЛЬКИМИ ВХОДЯЩИМИ ПОТОКАМИ
//     nodes.forEach((node, key) => {
//       // Пропускаем источники и уже обработанные узлы
//       if (node.isSource || processedNodes.has(node.id)) return;
//
//       // Суммируем входящие потоки
//       let totalIncoming = 0;
//       node.incoming.forEach(edge => {
//         totalIncoming += edge.flow || 0;
//       });
//
//       if (totalIncoming <= 0) return;
//
//       console.log(`Узел ${key}: сумма входящих потоков = ${totalIncoming.toFixed(3)} м³/с`);
//
//       // Учитываем сопротивление объекта в узле
//       let volumeForDistribution = totalIncoming;
//       if (node.objectResistance > 0) {
//         volumeForDistribution = roundTo5(totalIncoming / node.objectResistance);
//         console.log(`После объекта (R=${node.objectResistance}): ${volumeForDistribution.toFixed(3)} м³/с`);
//       }
//
//       // Распределяем по исходящим линиям
//       if (node.outgoing.length === 0) return;
//
//       let totalOutgoingConductivity = 0;
//       node.outgoing.forEach(edge => {
//         if (edge.resistance > 0) {
//           edge.conductivity = 1 / edge.resistance;
//           totalOutgoingConductivity += edge.conductivity;
//         }
//       });
//
//       if (totalOutgoingConductivity <= 0) return;
//
//       node.outgoing.forEach(edge => {
//         if (edge.resistance > 0) {
//           const conductivity = 1 / edge.resistance;
//           const flow = roundTo5(volumeForDistribution * (conductivity / totalOutgoingConductivity));
//
//           // Добавляем к существующему потоку (если есть)
//           const existingFlow = edge.flow || 0;
//           const newFlow = roundTo5(existingFlow + flow);
//
//           edge.flow = newFlow;
//           if (edge.line.properties) {
//             edge.line.properties.airVolume = newFlow;
//             edge.line.set('properties', edge.line.properties);
//           }
//
//           console.log(`Линия ${edge.id} получает дополнительно: ${flow} м³/с, всего: ${newFlow} м³/с`);
//
//           // Продолжаем распространение
//           const nextNode = nodes.get(edge.to);
//           if (nextNode && !processedNodes.has(nextNode.id)) {
//             processedNodes.add(nextNode.id);
//             // Не рекурсивно, чтобы избежать циклов
//           }
//         }
//       });
//     });
...
//     edges.forEach(edge => {
//       try {
//         if (edge.line.properties && edge.line.properties.airVolume !== undefined) {
...
//     });
//
//     // Дополнительно обновляем все тексты
...
//
//   // Создаем визуальные точки пересечения
//   intersections.forEach((inter, index) => {
//     createIntersectionPoint(inter.x, inter.y, index, inter);
//   });
//
//   // Создаем копию линий, чтобы не модифицировать исходный массив во время итерации
//   const linesCopy = [...getCachedLines()];
//
//   // Группируем пересечения по линиям
//   const intersectionsByLine = new Map();
//
//   intersections.forEach(inter => {
//     if (inter.line1) {
//       if (!intersectionsByLine.has(inter.line1)) {
//         intersectionsByLine.set(inter.line1, []);
//       }
//       intersectionsByLine.get(inter.line1).push(inter);
//     }
//     if (inter.line2) {
//       if (!intersectionsByLine.has(inter.line2)) {
//         intersectionsByLine.set(inter.line2, []);
//       }
//       intersectionsByLine.get(inter.line2).push(inter);
//     }
//   });
//
//   // Обрабатываем каждую линию
//   intersectionsByLine.forEach((intersections, line) => {
//     // Фильтруем и сортируем точки пересечения вдоль линии
//     const points = [];
//     const seen = new Set();
//
//     intersections.forEach(inter => {
...
//         seen.add(key);
//
//         // Вычисляем расстояние от начала линии
//         const distance = Math.sqrt(
...
//         // Создаем сегмент от текущего начала до точки пересечения
//         if (Math.abs(point.distance) > 0.1 &&
//           Math.abs(point.distance - line.properties?.length) > 0.1) {
//
//           const segment = splitLineAtPoint(line, {x: point.x, y: point.y});
//           if (segment) {
//             // Обновляем координаты первого сегмента
//             segment.line1.set({
//               x1: currentStart.x,
//               y1: currentStart.y,
//               x2: point.x,
...
//
//       // Заменяем исходную линию на сегменты
...
//     showNotification(`Найдено ${intersections.length} точек пересечения за ${(endTime - startTime).toFixed(0)}ms`, 'success');
//   } else {
...
//       airVolume1 = roundTo5(line.properties.airVolume);
//       airVolume2 = 0;
//     } else {
//       airVolume1 = roundTo5(line.properties.airVolume * proportion1);
//       airVolume2 = roundTo5(line.properties.airVolume * proportion2);
//     }
//   }
//
...
//   // Проверяем, не существует ли уже точка в этих координатах
...
//       // Определяем, является ли точка началом или концом линии
//       const distanceToStart = Math.sqrt(
//         Math.pow(x - line.x1, 2) + Math.pow(y - line.y1, 2)
//       );
//       const distanceToEnd = Math.sqrt(
//         Math.pow(x - line.x2, 2) + Math.pow(y - line.y2, 2)
//       );
//
...
//   }
//
//   // Добавляем расчет баланса воздуха в точке
//   if (pointData.linesInPoint && pointData.linesInPoint.length > 0) {
//     let totalIncoming = 0;
//     let totalOutgoing = 0;
//
...
//   if (!content) return;
//
//   // Находим координаты из текущего контента
...
//
//   // Пересчитываем информацию о точке
//   const pointInfo = collectPointInfo(x, y);
//   showIntersectionPointInfoModal(pointInfo);
//
...
//     stroke: '#4A00E0',
//     strokeWidth: 5,
//     properties: {name: 'Линия 1', airVolume: 0}
//   });
//
//   const line2 = new fabric.Line([200, 100, 200, 200], {
//     stroke: '#4A00E0',
//     strokeWidth: 5,
//     properties: {name: 'Линия 2', airVolume: 0}
//   });
//
//   const line3 = new fabric.Line([200, 100, 300, 100], {
//     stroke: '#4A00E0',
//     strokeWidth: 5,
//     properties: {name: 'Линия 3', airVolume: 0}
//   });
//
//   canvas.add(line1);
//   canvas.add(line2);
...
//
// console.log('Исправленный редактор технических чертежей загружен!');
...
function splitAllLines() {
  ...
  intersectionsByLine.forEach((intersections, line) => {
    ...
    if (points.length > 0) {
      ...
      for (const point of points) {
        // Пропускаем точки, которые слишком близко к началу или концу
        if (Math.abs(point.distance) > 0.1 &&
          Math.abs(point.distance - line.properties?.length) > 0.1) {

          // Создаем сегмент от currentStart до точки пересечения
          const segmentLength = Math.sqrt(
            Math.pow(point.x - currentStart.x, 2) + Math.pow(point.y - currentStart.y, 2)
          );
          const proportion = segmentLength / (line.properties?.length || 1);

          const segmentProps = {...line.properties};
          segmentProps.length = segmentLength;
          segmentProps.passageLength = roundTo5((line.properties?.passageLength || 0.5) * proportion);
          segmentProps.airResistance = calculateAirResistance(
            line.properties?.roughnessCoefficient || 0.015,
            segmentProps.perimeter || 0,
            segmentProps.passageLength,
            segmentProps.crossSectionalArea || 10
          );
          segmentProps.airVolume = roundTo5((line.properties?.airVolume || 0) * proportion);
          segmentProps.startPoint = {x: currentStart.x, y: currentStart.y};
          segmentProps.endPoint = {x: point.x, y: point.y};
      ...
      const lastSegmentLength = Math.sqrt(
        Math.pow(line.x2 - currentStart.x, 2) + Math.pow(line.y2 - currentStart.y, 2)
      );
      const lastProportion = lastSegmentLength / (line.properties?.length || 1);

      const lastSegmentProps = {...line.properties};
      ...
      lastSegmentProps.airResistance = calculateAirResistance(
        line.properties?.roughnessCoefficient || 0.015,
        lastSegmentProps.perimeter || 0,
        lastSegmentProps.passageLength,
        lastSegmentProps.crossSectionalArea || 10
      );
      lastSegmentProps.airVolume = roundTo5((line.properties?.airVolume || 0) * lastProportion);
      lastSegmentProps.startPoint = {x: currentStart.x, y: currentStart.y};
      lastSegmentProps.endPoint = {x: line.x2, y: line.y2};
...
function splitLineAtPoint(line, point) {
  ...
  // Если исходная линия была привязана к объекту в начале, то первая часть наследует эту привязку
  if (line.lineStartsFromObject && line.startObject && line.x1 === line1.x1 && line.y1 === line1.y1) {
    line1.lineStartsFromObject = true;
    line1.startObject = line.startObject;
    if (line1.properties) line1.properties.startsFromObject = line.properties?.startsFromObject;

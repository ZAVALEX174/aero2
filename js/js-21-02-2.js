// Полный код редактора с исправленным алгоритмом учёта сопротивления объектов на концах линий

console.log("ИСПРАВЛЕННАЯ ВЕРСИЯ: Расчёт воздуха с учётом сопротивления на концах линий");
...
/**
 * Находит все объекты (изображения) рядом с заданной точкой
 * @param {number} x - координата X
 * @param {number} y - координата Y
 * @param {number} threshold - радиус поиска
 * @returns {Array} массив объектов fabric.Image
 */
function findObjectsNearPoint(x, y, threshold = 30) {
  ...
  for (const img of images) {
    ...
    // Проверяем попадание точки в расширенную область прямоугольника объекта
...
/**
 * Корректирует потоки в исходящих рёбрах узла с учётом сопротивления объектов на концах линий.
 * Избыток (разница между исходным и скорректированным потоком) перераспределяется
 * между остальными исходящими рёбрами пропорционально их проводимости.
 * @param {Object} node - объект узла (с полем id для логирования)
 * @param {Array} outgoingEdges - массив объектов { line } (линии, начинающиеся в узле)
 */
function adjustFlowsForOutgoingEdges(node, outgoingEdges) {
  ...
  for (const edgeObj of outgoingEdges) {
    ...
    const objectsAtEnd = findObjectsNearPoint(endX, endY, 30);
    const objWithResistance = objectsAtEnd.find(obj => obj.properties && obj.properties.airResistance > 0);

    if (objWithResistance) {
      const originalFlow = line.properties.airVolume || 0;
      const resistance = objWithResistance.properties.airResistance;
      const correctedFlow = originalFlow / resistance;
      const excess = originalFlow - correctedFlow;

      corrections.push({
        line: line,
        correctedFlow: correctedFlow,
        excess: excess
      });
      totalExcess += excess;
    } else {
  ...
  // Распределяем excess между otherEdges
  if (totalExcess > 0 && otherEdges.length > 0) {
    // Вычисляем суммарную проводимость otherEdges
    let totalConductivity = 0;
    for (const edgeObj of otherEdges) {
      const line = edgeObj.line;
      const resistance = line.properties.airResistance || 1;
      totalConductivity += 1 / resistance;
    }
    ...
    if (totalConductivity > 0) {
      for (const edgeObj of otherEdges) {
        ...
        line.properties.airVolume = (line.properties.airVolume || 0) + additionalFlow;
      }
    }
  } else if (totalExcess > 0 && otherEdges.length === 0) {
    // Все исходящие линии ведут к объектам с сопротивлением — распределяем excess между ними же
    // (пропорционально скорректированным потокам, чтобы избежать рекурсии, просто предупреждаем)
    console.warn(`Узел ${node.id}: все исходящие линии ведут к объектам с сопротивлением, excess ${totalExcess.toFixed(3)} не распределён`);
  }

  // Применяем скорректированные потоки для линий с объектами
...
// Расчет потоков от источников (BFS от источников) с учётом сопротивления на концах линий
function calculateFlowsFromSources(graph, sources) {
  ...
  while (queue.length > 0) {
    ...
    if (node.processed) {
      ...
      // После перераспределения выполняем коррекцию для этого узла
      adjustFlowsForOutgoingEdges(node, node.outgoingEdges);
    ...
    // Если одна исходящая линия
    if (node.outgoingEdges.length === 1) {
      const edge = node.outgoingEdges[0];
      let flow = availableVolume;

      // Записываем предварительный поток
      edge.line.properties.airVolume = flow;

      // Вызываем коррекцию для этой линии (она может изменить поток, если на конце объект)
      adjustFlowsForOutgoingEdges(node, node.outgoingEdges);

      // Получаем окончательный поток
    ...
    // Несколько исходящих линий - распределяем по проводимости
      ...
      // Предварительно распределяем поток
      conductivities.forEach(item => {
        const flow = availableVolume * (item.conductivity / totalConductivity);
        // Пока не записываем в line.properties, сохраняем во временную переменную
        item.tempFlow = flow;
      });

      // Записываем временные потоки в свойства линий
      conductivities.forEach(item => {
      ...
      // Теперь для каждого исходящего ребра берем окончательный поток из line.properties и добавляем следующий узел
      node.outgoingEdges.forEach(edge => {
        const finalFlow = edge.line.properties.airVolume || 0;
...
// ==================== НОВАЯ ФУНКЦИЯ: ПРОВЕРКА ОБЪЕКТОВ НА КОНЦАХ ЛИНИЙ В ТОЧКАХ ПЕРЕСЕЧЕНИЯ ====================
function checkLineEndObjectsAtIntersections() {
  ...
  const intersections = intersectionPoints; // глобальный массив точек пересечения
  const images = getCachedImages();
  ...
  intersections.forEach((point, idx) => {
    ...
    // Находим линии, у которых начало совпадает с этой точкой (с допуском)
    const linesStartingHere = [];
    const lines = getCachedLines();
    lines.forEach(line => {
      const distToStart = Math.hypot(line.x1 - x, line.y1 - y);
      if (distToStart < 5) { // допуск 5 пикселей
        linesStartingHere.push(line);
      }
    });
    ...
    linesStartingHere.forEach((line, lineIdx) => {
      ...
      // Ищем объекты в конечной точке
      const objectsAtEnd = [];
      images.forEach(img => {
        const center = getObjectCenter(img);
        const distToEnd = Math.hypot(center.x - endX, center.y - endY);
        // Проверяем, попадает ли центр объекта в конечную точку (допуск 30 пикселей)
        if (distToEnd < 30) {
          objectsAtEnd.push(img);
        } else {
          // Дополнительно проверяем, лежит ли конечная точка внутри прямоугольника объекта
          const rect = getObjectRect(img);
      ...
      if (objectsAtEnd.length > 0) {
        ...
        objectsAtEnd.forEach((obj, objIdx) => {
          const objName = obj.properties?.name || 'Объект без имени';
          console.log(`      - ${objName} (тип: ${obj.properties?.type || 'неизвестен'})`);
        });
      } else {
...
// ==================== АНАЛИЗ ТОЧЕК ПЕРЕСЕЧЕНИЯ (МОДИФИЦИРОВАН) ====================
function analyzeIntersectionPoints() {
  ...
  for (const [key, point] of pointsMap.entries()) {
    ...
    // ===== ДОБАВЛЕННАЯ КОРРЕКЦИЯ ДЛЯ УЧЁТА СОПРОТИВЛЕНИЯ НА КОНЦАХ ЛИНИЙ =====
    // После применения всех логик для узла, выполняем коррекцию потоков для исходящих линий с учётом объектов на концах
    if (point.linesStarting.length > 0) {
      // Создаем временный массив объектов, похожих на edge, с полем line
      const outgoingEdges = point.linesStarting.map(item => ({line: item.line}));
      adjustFlowsForOutgoingEdges({id: key}, outgoingEdges);
    }
    // =======================================================================
  }
  ...
  // Вызываем новую проверку объектов на концах линий
...
console.log('Редактор технических чертежей загружен с исправленным алгоритмом учёта сопротивления на концах линий!');
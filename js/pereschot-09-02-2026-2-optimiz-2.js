...
function splitLineAtPoint(line, point) {
  ...
  // Проверяем, что точка действительно на линии
  const closestPoint = findClosestPointOnLine(point, line);
  if (closestPoint.distance > 10) {
    return null; // Точка слишком далеко от линии
  }
  ...
  // Проверяем минимальную длину сегментов
  ...
  // Обновляем свойства для каждой части
  ...
  // Вычисляем сопротивление для каждой части
  ...
  // Распределяем объем воздуха пропорционально длине
  if (props.airVolume !== undefined) {
    if (line.lineStartsFromObject && line.startObject) {
      // Если линия начинается от объекта, первая часть получает весь объем
  ...
  // Удаляем любые связи с исходной линией
  delete props1.startsFromObject;
  delete props2.startsFromObject;

  // Создаем новые независимые линии
  ...
  // Если исходная линия начиналась от объекта, сохраняем эту связь только для первой части
...
function handleLineDrawingEnd(options, pointer) {
  ...
  // Разделяем существующую линию, если мы пересекаем её
  if (!altKeyPressed) {
...
function splitAllLines() {
  ...
  linesCopy.forEach(line => {
    ...
    // Собираем все точки пересечения для этой линии
    intersections.forEach(inter => {
      if (inter.line1 === line || inter.line2 === line) {
        ...
        // Проверяем, не добавляли ли уже эту точку
    ...
    // Если есть точки пересечения, разделяем линию
    if (points.length > 0) {
      ...
      for (const point of points) {
        // Создаем сегмент от текущей начальной точки до точки пересечения
        const segmentLength = Math.sqrt(
          Math.pow(point.x - currentStart.x, 2) +
          Math.pow(point.y - currentStart.y, 2)
        );

        if (segmentLength > 5) { // Минимальная длина сегмента
          const proportion = segmentLength / line.properties?.length || 1;

          // Создаем независимый сегмент
      ...
      // Если создали более одного сегмента, заменяем исходную линию
...
function analyzeIntersectionPoints() {
  ...
  // Обрабатываем каждый узел по правилам
  ...
  for (const [key, point] of pointsMap.entries()) {
    ...
    // Суммируем все входящие объемы для СЛУЧАЯ 7

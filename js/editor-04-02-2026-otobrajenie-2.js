...
function calculateAirVolumesForAllLines() {
  try {
    ...
    // Безопасный обход линий
    lines.forEach(line => {
      try {
        if (line.lineStartsFromObject && line.startObject) {
          const obj = line.startObject;
          if (obj && obj.properties && obj.properties.airVolume !== undefined && obj.properties.airVolume !== null) {
        ...
        console.warn('Error processing line with startObject:', err, line);
      }
    });

    for (const [key, pointData] of intersectionPointsMap.entries()) {
      try {
        // КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: Проверяем элементы массивов
        if (pointData.linesEnding.length === 1 && pointData.linesStarting.length === 1) {
          ...
          if (!endingLine || !startingLine) continue;

          if (endingLine?.properties?.airVolume !== undefined && endingLine.properties.airVolume !== null) {
        ...
        if (pointData.objects.length > 0 && pointData.linesStarting.length > 0) {
          pointData.objects.forEach(obj => {
            ...
            if (obj.properties.airVolume !== undefined && obj.properties.airVolume !== null) {
        ...
        console.warn('Error processing intersection point:', err, pointData);
      }
    }
    ...
    if (linesToUpdate.size > 0) {
      ...
      console.log('=== calculateAirVolumesForAllLines completed successfully ===');
      return true;
    }

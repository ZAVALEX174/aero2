...
// // Вставьте это в самый верх вашего скрипта перед инициализацией canvas
// try {
...
//     set: function (value) {
//       // Если Fabric пытается пропихнуть 'alphabetical', мы меняем его на валидное
...
//   // Добавляем обработчик для кнопки расчета воздуха
...
// // Подавление ошибки CanvasTextBaseline
// (function () {
...
//     const airVolumeText = new fabric.Text(`${line.properties.airVolume} м³/с`, textOptions);
...
//   lines.forEach(line => createOrUpdateAirVolumeText(line));
//   canvas.renderAll();
// }
...
// // ==================== УЛУЧШЕННАЯ ФУНКЦИЯ РАСЧЕТА ОБЪЕМА ВОЗДУХА ДЛЯ ЛИНИЙ ====================
// function calculateAirVolumesForAllLines(isManual = false) {
//   // Расчет выполняется ТОЛЬКО при явном вызове через кнопку (isManual = true)
//   if (!isManual) {
//     return false;
//   }
...
//   showNotification('Начинается расчет объемов воздуха...', 'info', 5000);

//   const lines = canvas.getObjects().filter(obj => obj.type === 'line' && obj.id !== 'grid-line');
//   const images = canvas.getObjects().filter(obj => obj.type === 'image' && obj.properties);

//   console.log(`Найдено ${lines.length} линий и ${images.length} объектов-изображений`);

//   // Найти начальные точки (линии, начинающиеся от объектов)
//   const startPoints = [];
//   lines.forEach(line => {
//     if (line.lineStartsFromObject && line.startObject) {
//       const startPoint = { x: line.x1, y: line.y1, line: line, object: line.startObject };
//       startPoints.push(startPoint);
//       console.log(`Найдена линия, начинающаяся от объекта: ${line.properties?.name || 'Без названия'}`);
//     }
//   });

//   // Для каждой начальной точки установить объем воздуха из объекта
//   startPoints.forEach(point => {
//     if (point.object.properties && point.object.properties.airVolume !== undefined) {
//       point.line.properties.airVolume = roundTo5(point.object.properties.airVolume);
//       point.line.set('properties', point.line.properties);
//       createOrUpdateAirVolumeText(point.line);
//       console.log(`Установлен объем воздуха для линии ${point.line.properties?.name || 'Без названия'}: ${point.line.properties.airVolume} м³/с`);
//     }
//   });

//   // Создать граф для расчета
//   const nodes = [];
//   const edges = [];

//   // Собрать все уникальные точки (начала и концы линий)
//   lines.forEach(line => {
//     const startKey = `${roundTo5(line.x1)}_${roundTo5(line.y1)}`;
//     const endKey = `${roundTo5(line.x2)}_${roundTo5(line.y2)}`;

//     if (!nodes.find(n => n.key === startKey)) {
//       nodes.push({ key: startKey, x: roundTo5(line.x1), y: roundTo5(line.y1) });
//     }
//     if (!nodes.find(n => n.key === endKey)) {
//       nodes.push({ key: endKey, x: roundTo5(line.x2), y: roundTo5(line.y2) });
//     }
...
//       to: endKey,
//       line: line,
//       resistance: line.properties?.airResistance || 0
//     });
//   });

//   // Алгоритм расчета объемов воздуха
//   const processedLines = new Set();
//   const queue = [...startPoints];

//   while (queue.length > 0) {
//     const current = queue.shift();
//     const currentPointKey = `${roundTo5(current.x)}_${roundTo5(current.y)}`;

//     // Найти все линии, выходящие из этой точки
//     const outgoingEdges = edges.filter(edge =>
//       edge.from === currentPointKey && !processedLines.has(edge.line)
//     );

//     if (outgoingEdges.length > 0) {
//       let totalResistance = outgoingEdges.reduce((sum, edge) => sum + (edge.resistance || 1), 0);

//       outgoingEdges.forEach(edge => {
//         const line = edge.line;
//         const startPointVolume = current.line ? current.line.properties?.airVolume :
//           (current.object ? current.object.properties?.airVolume : 0);

//         if (startPointVolume !== undefined && startPointVolume !== null) {
//           // Распределение объема пропорционально сопротивлению
//           const resistanceRatio = totalResistance > 0 ?
//             (edge.resistance || 1) / totalResistance : 1 / outgoingEdges.length;

//           line.properties.airVolume = roundTo5(startPointVolume * resistanceRatio);
//           line.set('properties', line.properties);
//           createOrUpdateAirVolumeText(line);
//           processedLines.add(line);

//           // Добавить конечную точку этой линии в очередь
//           const endKey = edge.to;
//           const endNode = nodes.find(n => n.key === endKey);
//           if (endNode) {
//             queue.push({
//               x: endNode.x,
//               y: endNode.y,
...
//   // Для линий без источника установить значение по умолчанию или 0
//   lines.forEach(line => {
//     if (!processedLines.has(line) && (line.properties.airVolume === undefined || line.properties.airVolume === null)) {
...
//   showNotification(`Расчет объемов воздуха завершен! Обработано ${lines.length} линий`, 'success');
...
//   // Временно отключаем пересчет при модификации
// }
...
//       ? formatTo5(line.properties.airVolume) : 'не задан';
//     let sourceInfo = '';
//     if (line.lineStartsFromObject && line.startObject) {
...
//   notification.innerHTML = `<span>${getNotificationIcon(type)}</span><span>${message}</span>`;
...
//       if (item.text && !item.text.isRemoved) item.text.visible = item.visible;
//     });
//     canvas.renderAll();
//   }
// }
...
//   const wasGridVisible = gridVisible;
//   if (!includeGrid && wasGridVisible) toggleGrid();
//   else if (includeGrid && !wasGridVisible) toggleGrid();
...
//       try {
//         const { jsPDF } = window.jspdf;
//         let pdf, imgWidth, imgHeight, x, y;
...
//         } else {
//           const formatDimensions = { 'A4': [210, 297], 'A3': [297, 420] };
...
//             const objectCount = canvas.getObjects().filter(obj => obj.id !== 'grid-group' && obj.id !== 'grid-line').length;
...
//         showNotification('Ошибка при создании PDF: ' + error.message, 'error');
//       } finally {
//         if (wasGridVisible !== gridVisible) toggleGrid();
//         originalTextsVisibility.forEach(item => {
//           if (item.text && !item.text.isRemoved) item.text.visible = item.visible;
//         });
//         canvas.renderAll();
...
// document.addEventListener('DOMContentLoaded', function () {
//   const canvasElement = document.getElementById('fabric-canvas');
//   if (canvasElement) canvasElement.addEventListener('contextmenu', function (e) {
//     e.preventDefault();
//   });
// });
...
function calculateAirVolumesForAllLines(isManual = false) {
  ...
  try {
    ...
    // Шаг 9: Рендерим и показываем результат
    canvas.renderAll();
    updatePropertiesPanel();
...
function exportLinePropertiesToCSV() {
  ...
  let csvContent = "Название,Длина (px),Длина прохода (м²),Коэффициент шероховатости,Площадь сечения (м),Периметр,W (кг/м),Воздушное сопротивление,Объем воздуха (м³/с),Цвет,Толщина\n";
